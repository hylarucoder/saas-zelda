package xyz.zelda.gateway.core.mappings
...
class MappingsValidator {
    fun validate(mappings: List<MappingProperties>) {
        if (!CollectionUtils.isEmpty(mappings)) {
            ...
            mappings.sortedWith(java.util.Comparator { mapping1: MappingProperties, mapping2: MappingProperties -> mapping2.host.compareTo(mapping1.host) })
        }
    }
    ...
    protected fun validateDestinations(mapping: MappingProperties?) {
        ...
        mapping.destinations.forEach(Consumer { destination: String ->
            ...
            if (!destination.matches(".+://.+".toRegex())) {
